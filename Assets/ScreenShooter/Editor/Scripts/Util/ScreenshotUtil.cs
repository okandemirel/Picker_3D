...
namespace Borodar.ScreenShooter.Utils
{
    public static class ScreenshotUtil
    {
        public static void TakeScreenshot(ScreenShooterSettings settings, ScreenshotConfig config)
        {
            ...
            TakeScreenshot(settings.Camera,settings, settings.SaveFolder, settings.Tag, suffix, config);
        }

        public static void TakeScreenshot(Camera camera,ScreenShooterSettings settings, string folderName, string prefix, string suffix, ScreenshotConfig screenshotConfig)
        {
            ...
            SaveTextureAsFile(scrTexture,settings ,folderName, prefix, suffix, screenshotConfig);
        }
        
        public static void SaveTextureAsFile(Texture2D texture, ScreenShooterSettings settings, string folder, string prefix, string suffix, ScreenshotConfig screenshotConfig)
        {
            byte[] bytes;
            string extension;
