...
namespace AssetInventory
{
    public sealed class DependenciesUI : EditorWindow
    {
        ...
        public void OnGUI()
        {
            EditorGUILayout.LabelField($"{_info.Dependencies.Count} dependencies of '{_info.FileName}' in asset '{_info.GetDisplayName(false)}' ({EditorUtility.FormatBytes(_info.Size)})", EditorStyles.boldLabel);
            ...
            foreach (AssetFile info in _info.Dependencies.OrderBy(f => f.Type).ThenBy(f => f.Path))
            {
                GUILayout.BeginHorizontal();
                if (info.InProject)
                {
                ...
                EditorGUILayout.LabelField(new GUIContent(info.Path + " (" + EditorUtility.FormatBytes(info.Size) + ")"), _info.ScriptDependencies.Contains(info) ? UIStyles.ColoredText(Color.yellow) : EditorStyles.label);
                GUILayout.EndHorizontal();
            }
